...
defer db.Close()
 ...
 tx, err := db.Begin()
    ...
    for i := 0; i < len( global_pool.slice); i++ {
        pool_str :=  global_pool.slice[i]
	tokens := strings.Split( pool2string( pool_str)  , " ")
        item.Id =  strconv.Itoa(i)  
        item.XmlUrl = tokens[0]
        item.UrlInfo = strings.Join(tokens[1:], " ")
        item.Ph1 = 0
        item.Ph2 = 0
        item.Ph3 = 0
...
f.Write([]byte( "//lock db " + myt() + ", set LOCKDBFLAG"  +"\n"  ));
...
//global_pool = NewSyncList(0)
global_pool = nil 
f.Write([]byte( "//unlock db " + myt() + ", rm  LOCKDBFLAG"  +"\n"  ));
